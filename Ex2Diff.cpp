...
//課題プログラム2: 目標への移動＋障害物回避
//2021/6/15 HT21A099 南　李玖
#include "ise-robot.h"
#include <iostream>
#include <cmath>
#include <vector>
...
void loop()
{
    ...
    if (state == Stop) {
        ...
        if (nearest.x > 0.4 && abs(nearest.y) >= 0.15) { //障害物が少し遠くなれば
            state = Avoid;
        ...
        if (currentTime - avoidTime > 2.5 && nearest.x >= 0.3 && abs(nearest.y) <= 0.3) { //Avoidを始めてから一定時間経過していれば
            state = Turn;
            cout << "Avoid -> Turn" << endl;
        }
        else if (nearest.x < 0.35 && abs(nearest.y) <= 0.3) { //障害物が近ければ
            state = Search;
